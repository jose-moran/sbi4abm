...
def create_density_estimator(embedding_kwargs, density_estimator):
    ...
    if density_estimator in ["maf", "nsf", "made", "mdn"]:
        density_estimator = utils.posterior_nn(
            model=density_estimator, embedding_net=embedding_net, z_score_x=z_score_x
        )
    # Else it's a ratio estimator (classifier), so just use default settings again
    elif density_estimator in ["linear", "mlp", "resnet"]:
        density_estimator = utils.get_nn_models.classifier_nn(
            density_estimator, embedding_net_x=embedding_net, z_score_x=z_score_x
        )
...
def sbi_training(
    ...
    for sim_count in n_sims:
        theta, x = simulate_for_sbi(
            sbi_simulator, proposal, num_simulations=sim_count, num_workers=num_workers
        )
    ...
    if n_samples > 0:
        if sampler is None:
            ...
            weights = np.exp(
                proposal.log_prob(prior_samples).numpy()
                - prior.log_prob(prior_samples).numpy()
            )
            weights = weights.reshape(-1)
            idx = np.random.choice(
                np.arange(weights.shape[0]), p=weights / np.sum(weights), size=n_samples
            )
