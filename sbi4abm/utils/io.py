...
class Simulator:
    ...
    def __call__(self, pars):
        ...
        x = self.model.simulate(pars=pars, t_max=self.T)
        if len(x.shape) == 1:
...
def _load_simulator(task_name):
    ...
    if task_name == "bh_noisy":
        model = BrockHommes.Model(beta=10.0)
    elif task_name == "bh_smooth":
        model = BrockHommes.Model(beta=120.0)
        ...
        initial_state = np.load(
            os.path.join(this_dir, "../data/Hopfield/small_graph_correct.npy")
        )[0]
...
def _load_prior(task_name):
    ...
    if task_name == "bh_noisy":
        prior = utils.BoxUniform(
            low=torch.tensor([-1.0, -1.0, 0.0, 0.0]),
            high=torch.tensor([0.0, 0.0, 1.0, 1.0]),
        )
    elif task_name == "bh_smooth":
        prior = utils.BoxUniform(
            low=torch.tensor([0.0, 0.0, 0.0, -1.0]),
            high=torch.tensor([1.0, 1.0, 1.0, 0.0]),
        )
    elif task_name == "fw_hpm":
        prior = utils.BoxUniform(
            low=torch.tensor([-1.0, 0.0, 0.0, 0.0]),
            high=torch.tensor([1.0, 2.0, 20.0, 5.0]),
        )
    elif task_name == "fw_wp":
        prior = utils.BoxUniform(
        ...
        prior = utils.BoxUniform(
            low=torch.tensor([0.0, 0.0, 0.0]), high=torch.tensor([5.0, 1.0, 1.0])
        )
    elif task_name == "mvgbm":
        prior = utils.BoxUniform(
            low=torch.tensor([-1.0, -1.0, -1.0]), high=torch.tensor([1.0, 1.0, 1.0])
        )
    return prior
...
def _load_true_pars(task_name):
    ...
    if task_name == "bh_noisy":
        ...
        theta = np.array([2668 / 15000.0, 0.987 / 1.0, 1.726 / 5.0])
    elif task_name == "hop":
        theta = np.array([1.0, 0.8, 0.5])
    elif task_name == "mvgbm":
        theta = np.array([0.2, -0.5, 0.0])
    return theta
