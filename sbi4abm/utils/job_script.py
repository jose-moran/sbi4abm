...
def _neural_prepare_estimator_observation(args, y, simulator):
    ...
    density_estimator, z_score_x = neural.create_density_estimator(
        embedding_kwargs, de_name
    )
...
def _raise_kde_warnings(args, nsims):
    ...
    if len(nsims) > 2:
        warning = (
            "Multi-round inference not compatible with "
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Job script for sbi4abm", formatter_class=RawTextHelpFormatter
    )
    ...
    parser.add_argument(
        ...
        type=str,
        help=(
            "Name of density estimator/classifier. Suffix "
    ...
    parser.add_argument(
        ...
        nargs="+",
        help=(
            "Number of simulations to run, e.g. 1000 1000"
    ...
    parser.add_argument(
        "--outloc", type=str, help="Location to dump inference results."
    )
    parser.add_argument(
        "--scale", type=float, help="Scale (> 0) for MCMC proposal covariance."
    )
    parser.add_argument(
        "--R",
        type=int,
    ...
    parser.add_argument(
        ...
        default="mh",
        help=(
            "Which sampling method to use for (S)NRE for the "
    ...
    if args.method[:3] == "kde":
        ...
        posteriors, samples = kde.kde_training(
            simulator,
            prior,
            y,
            start=true_theta,
    ...
    ##################
        ...
        if args.sbc and isinstance(nsims, list):
            if len(nsims) > 1:
                error_msg = (
                    "Currently no implementation of SBC with "
        ...
        density_estimator, sbi_method, y, simulator, sim_pp, z_score_x = (
            _neural_prepare_estimator_observation(args, y, simulator)
        )
        ...
        if len(args.load_post) == 0:
            posteriors, samples = neural.sbi_training(
                simulator,
                prior,
                y,
                sbi_method,
